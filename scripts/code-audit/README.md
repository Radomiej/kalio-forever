...
- `docs/audit/<YYYY-MM-DD>-report.md` — human-readable prioritized refactor queue
  following AGENTS.md architecture rules. The JSON report keeps the complete
  architecture-debt set even when the Markdown ranking shows only the first 25.
...
- 🔴 CRITICAL — silent catch in critical path, circular dep > 3 modules
- 🟡 HIGH     — hard-limit architecture debt, silent catch non-critical, circular dep
- 🟢 MEDIUM   — soft-limit architecture debt, `any` hotspot (≥ 5/file), duplicate clone, unused export, free-form text parsing branch
