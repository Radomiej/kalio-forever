...
- Provider history must exclude messages owned by durable terminal `failed`, `cancelled`, or `interrupted` turns while retaining them in the user-visible transcript. Use `turnId` and `chat_runs.status`; never infer this boundary from text or elapsed time.
- Paid/live QA must validate the request-scoped model of the persona or slot being exercised, not only the global credential model. Use the required-persona readiness options when a specific model is claimed.
- Provider combinations such as tools plus structured output must be selected from explicit provider capabilities. Never retry by matching an error message; when structured output is authoritative, a provider that forbids the combination receives no tools for that request.
