...
This table is the current gap list. Historical implementation notes are kept lower in this document under implementation status.

| Gap | Severity | Current evidence | Required work |
| --- | --- | --- | --- |
| Native `run_sub_agentflow` still lacks final paid proof | Medium | Tool metadata, validation, dispatch integration, return modes, copy-back, parent result rendering, Canvas child projection, and FE-start mock E2E coverage exist. | After readiness gates stay green and the live provider is valid, run one paid real-project proof from Kalio FE and capture screenshots/logs. |
| Explicit child-execution kind adoption still has two naming layers | Medium | `ChildExecutionKind` exposes parent-facing `sub_agentflow`; runtime snapshots use `RuntimeChildExecutionKind = 'agent_flow'` and FE graph projection maps it back to `sub_agentflow`. | Keep the mapping covered by tests and continue replacing ad hoc child-kind strings in audit/projection surfaces. Do not rename the runtime kind without a contract migration. |
...
| Return modes need ongoing UI guardrails | Low | `summary`, `full_trace`, and `artifacts_only` now have executable adapter semantics. | Keep tool-result and Canvas/Graph projections aligned with those semantics when new AgentFlow surfaces are added. |
...
| VFS copy-back for flow children needs live proof | Low | `run_sub_agentflow` supports copy-back through the runtime/tool entrypoint and focused tests cover isolated artifact copy-back. | Prove copy-back in one live/manual AgentFlow run before using it as release evidence. |
| Two-agent target naming requires compatibility guard | Low | `goal_guard_delivery_loop` is aliased to the current `goal-master-delivery-loop` runtime schema. | Keep the alias until any schema rename has a migration and persisted-run compatibility story. |
