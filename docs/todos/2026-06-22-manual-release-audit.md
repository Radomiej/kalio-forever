...
- 2026-06-22: The only blocker still observed during manual QA is the already-known stack-state drift: after reactivating OpenRouter on fixed QA, `/api/llm/config` correctly reports `openrouter / cohere/north-mini-code:free / db`, while `node scripts/stack-manager.mjs status --json` still reports stale `xiaomimimo / mimo-v2.5-pro`.
