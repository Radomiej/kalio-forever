...
- 2026-06-22: Added a dedicated second-UI replay proof on current fixed QA. `corepack pnpm --filter @kalio/e2e exec playwright test tests/architecture-multi-ui-replay.spec.ts --project=chromium` passed on external `3316/5288` and proved that a fresh browser session can reopen the same host workflow, see completed node/agent statuses, list branch + technical child sessions, and open both a branch transcript and a technical router/finalizer transcript without the `Waiting for the first persisted message` placeholder.
