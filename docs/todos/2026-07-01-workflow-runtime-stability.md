...
- 2026-07-03: Recorded the architectural rule that fixed-duration waits are not runtime proof. Removed the `release:workflow-gate` audit quiet-time wait and added a script regression that blocks reintroducing fixed quiet-time waits as runtime evidence. Real `mimo-v2.5-pro` browser smoke now fails fast with typed `PROVIDER_UNAUTHORIZED` when the configured env credential is rejected; mock workflow release gate passed 17/17 browser/runtime scenarios on the managed QA stack.
- 2026-07-03: Fresh release-readiness verification after removing quiet-time waits passed: `npm.cmd run test` (script tests 58/58, API 2480/2480, web 1619/1619, launcher 12/12), `npm.cmd run typecheck`, `npm.cmd run build`, `node scripts\workflow-release-gate.mjs --reuse-stack` (17/17 browser/runtime scenarios), and `npm.cmd run audit:report` (CRITICAL 25, HIGH 0, MEDIUM 67, LOW 71).
- 2026-07-03: Strengthened live workflow release gating. `workflow-release-gate --require-live` now runs `scripts/agentflow-paid-readiness.mjs --api <qa-api>/api` after provider/source/credential checks and before browser workflow groups, so invalid live credentials fail before expensive workflow execution. RED/GREEN script regression passed (`node --test scripts\runtime-scripts.test.mjs` 18/18), and mock `node scripts\workflow-release-gate.mjs --reuse-stack` still passed 17/17 browser/runtime scenarios.
- 2026-07-03: Expanded `workflow-release-gate` to include existing sequential router-chain and cross-browser replay scenarios. The gate now covers a custom `router -> role -> router -> role -> router -> artifact` variant and a second-browser durable replay of host state, child transcripts, and technical node notes. Script regression passed and `node scripts\workflow-release-gate.mjs --reuse-stack` passed 19/19 scenarios.
- 2026-07-03: Hardened inline architecture schema validation. Runtime DTO validation now rejects node `roleSlotId` values that do not exist in `roleSlots` and duplicate role slot ids; direct RED/GREEN coverage also locks invalid role/router/artifact behavior combinations, duplicate node ids, duplicate edge ids, and self-loop edges. Fresh verification passed: focused schema/runtime tests (99 tests), `kalio-api` typecheck, `node --test scripts\runtime-scripts.test.mjs` (18/18), root `npm.cmd run test` (script 59/59, API 2482/2482, web 1619/1619, launcher 12/12), `npm.cmd run audit:report` (CRITICAL 25, HIGH 0, MEDIUM 67, LOW 71), root `npm.cmd run build`, and post-build `node scripts\workflow-release-gate.mjs --reuse-stack` (19/19 browser/runtime scenarios).
- 2026-07-03: Added the project learning that fixed-duration waits are never an architectural runtime solution. Sleeps may remain only as bounded diagnostics/test fallbacks; product/runtime correctness must use typed state, drain/ack barriers, durable snapshots, or explicit lifecycle events.
- 2026-07-03: Closed the Architect UI variant -> Talk workflow gap. `maxToolAttempts` and tool override draft changes now enable variant save, saved/deleted architecture registry entries emit a browser invalidation event, and Talk refreshes its architecture options from the backend snapshot instead of relying on a stale one-time list. Focused unit tests passed (`ArchitectPage.test.tsx` 23/23, `ChatInterface.test.tsx` 92/92), focused Playwright passed for `saves an Architect UI variant and runs it through Talk workflow mode`, and fresh `node scripts\workflow-release-gate.mjs` exited 0 across the 12 configured browser/runtime groups on a new mock QA stack.
