...
- Exception: `VFSModule`, `ToolModule`, `HitlModule`, and `RelayModule` are explicitly imported by `ChatModule`. Tools in `ToolModule` may also directly inject services from globally-registered modules (e.g. `RelayService` in `EscalateTool`).
