...
async function saveHitlMode(page: Page, mode: Extract<HitlMode, 'manual' | 'bypass'>): Promise<void> {
  ...
  if (mode === 'manual') {
    ...
    await page.getByRole('radio', { name: 'Bypass all' }).check();
  }
...
async function expectHitlAuditEvent(
  request: APIRequestContext,
  sessionId: string,
  eventType: string,
): Promise<void> {
  await expect
    .poll(async () => {
      const response = await request.get(`${API_BASE}/audit-log?limit=100&type=external_hitl`);
      expect(response.ok()).toBeTruthy();
      const rows = await response.json() as AuditRow[];
      return rows.some((row) => (
        row.sessionId === sessionId
        && row.type === 'external_hitl'
        && row.data?.domain === 'hitl'
        && row.data?.eventType === eventType
      ));
    }, {
      timeout: 10_000,
      message: `Expected HITL audit event ${eventType} for session ${sessionId}`,
    })
    .toBe(true);
}
...
test.describe('HITL settings modes', () => {
  ...
  test('manual mode shows tool confirmation and RA-App approval overlay', async ({ page, request }) => {
    ...
    try {
      ...
      await expectHitlAuditEvent(request, raappSession.id, 'raapp_approval_executed');
    } finally {
