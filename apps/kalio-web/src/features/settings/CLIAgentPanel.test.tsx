...
describe('CLIAgentPanel', () => {
  ...
  it('saves a trimmed model override for model-capable agents', async () => {
    ...
    const modelInput = await screen.findByPlaceholderText('e.g. gpt-5.4-mini');
    fireEvent.change(modelInput, { target: { value: '  gpt-5.4-mini  ' } });
    await user.click(screen.getByRole('button', { name: 'Save' }));

    await waitFor(() => {
      expect(getPutBody('codex').model).toBe('gpt-5.4-mini');
    });
  });

  it('saves a trimmed architecture run preference', async () => {
    installFetchMock();
    render(<CLIAgentPanel />);

    const preferenceInput = await screen.findByLabelText('Architecture run preference');
    fireEvent.change(preferenceInput, {
      target: { value: '  Prefer cheap materialization during architecture runs.  ' },
    });
    fireEvent.click(screen.getByText('Save'));

    await waitFor(() => {
      expect(getPutBody().architecturePreference).toBe('Prefer cheap materialization during architecture runs.');
    });
  });
