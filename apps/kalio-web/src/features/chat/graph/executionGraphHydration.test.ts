...
describe('extractExecutionGraphHydrationStatus', () => {
    ...
    const status = extractExecutionGraphHydrationStatus(parentMessages, {
      'branch-1': [
        ...
        message({
          ...
          content: JSON.stringify({
            status: 'error',
            errorCode: 'VFS_FILE_NOT_FOUND',
            errorMessage: "ENOENT: no such file or directory, open 'project/README.md'",
          }),
        }),
      ],
    });
  ...
  it('does not infer failed VFS reads from free-form tool result text', () => {
    const parentMessages = [
      message({
        id: 'result-1',
        role: 'tool_result',
    ...
    const status = extractExecutionGraphHydrationStatus(parentMessages, {
      'branch-1': [
        ...
        message({
          ...
          role: 'tool_result',
          toolCallId: 'read-1',
          content: "Diagnostic text mentions ENOENT and VFS_FILE_NOT_FOUND but has no typed error payload.",
        }),
      ],
    });
