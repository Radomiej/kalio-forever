...
function getStoredSessionMessages(state: SessionState, sessionId: string | null): ChatMessage[] {
  if (!sessionId) return [];
  return state.sessionMessages[sessionId] ?? (sessionId === state.activeSessionId ? state.messages : []);
}

function getStoredSessionTurns(state: SessionState, sessionId: string | null): AgentTurn[] {
  if (!sessionId) return [];
  return state.sessionAgentTurns[sessionId] ?? (sessionId === state.activeSessionId ? state.agentTurns : []);
}

function getStoredSessionActiveTurnId(state: SessionState, sessionId: string | null): ID | null {
...
function resolveSessionSlice(state: SessionState, sessionId: string | null): { messages: ChatMessage[]; agentTurns: AgentTurn[]; activeTurnId: ID | null } {
  if (!sessionId) {
