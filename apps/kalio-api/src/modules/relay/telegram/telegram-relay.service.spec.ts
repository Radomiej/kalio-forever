...
describe('TelegramRelayService', () => {
  ...
  it('executes registered telegram command callbacks', async () => {
    ...
    const commandHandlers = {
      ...
      cancelToolConfirmation: vi.fn(async () => 'Cancelled HITL request req-2.'),
    };
    ...
    const approveHandler = commandCalls.find((call) => call[0] === 'approve')?.[1] as
      ...
      | undefined;
    const cancelHandler = commandCalls.find((call) => call[0] === 'cancel')?.[1] as
      | ((ctx: CommandContext & { match?: string }) => Promise<void>)
      | undefined;
    ...
    await approveHandler!({ chat: { id: 42 }, reply, match: 'req-1 safe' });
    await cancelHandler!({ chat: { id: 42 }, reply, match: 'req-2 unsafe' });

    expect(settings.set).toHaveBeenCalledWith('relay.telegram.chat_id', '42');
    expect(commandHandlers.getStatus).toHaveBeenCalledOnce();
    expect(commandHandlers.stopAll).toHaveBeenCalledOnce();
    expect(commandHandlers.approveToolConfirmation).toHaveBeenCalledWith('req-1', 'safe');
    expect(commandHandlers.cancelToolConfirmation).toHaveBeenCalledWith('req-2', 'unsafe');
    expect(reply).toHaveBeenCalledWith(
    ...
    expect(reply).toHaveBeenCalledWith('Approved HITL request req\\-1\\.', { parse_mode: 'MarkdownV2' });
    expect(reply).toHaveBeenCalledWith('Cancelled HITL request req\\-2\\.', { parse_mode: 'MarkdownV2' });
  });

  it('routes registered chat text approval replies through command handlers', async () => {
    const service = new TelegramRelayService(settings as never);
    const reply = vi.fn(async () => undefined);
    const commandHandlers = {
      getStatus: vi.fn(async () => 'All systems nominal'),
      ...
      handleApprovalReply: vi.fn(async (text: string) => text.startsWith('approve')
        ? 'Approved HITL request req-text.'
        : null),
    };
    ...
    await messageHandler!({ chat: { id: 42 }, reply });
    await messageHandler!({ chat: { id: 42 }, reply, message: { text: 'approve req-text safe' } });
    await messageHandler!({ chat: { id: 42 }, reply, message: { text: 'hello' } });

    expect(commandHandlers.handleApprovalReply).toHaveBeenCalledWith('approve req-text safe');
    expect(reply).toHaveBeenCalledWith('Approved HITL request req\\-text\\.', { parse_mode: 'MarkdownV2' });
    expect(reply).toHaveBeenCalledWith(
