...
import type { CLIAgentRunLimits, ProgressEmitFn } from './cli-agent.types';
...
export class CLIAgentPtyService {
  ...
  async run(request: CLIAgentPtyRunRequest): Promise<CLIAgentResult> {
    ...
    return new Promise<CLIAgentResult>((resolve, reject) => {
      ...
      const rejectForTimeout = (kind: 'idle' | 'hard'): void => {
        if (settled) {
          return;
        }
        settled = true;
        ...
        reject(new Error(formatTimeoutError(request.agentId, {
      ...
      const resetIdleTimer = (): void => {
        if (idleTimer) clearTimeout(idleTimer);
        idleTimer = setTimeout(() => rejectForTimeout('idle'), request.inactivityTimeoutMs);
      };

      resetIdleTimer();
      if (request.hardTimeoutMs !== undefined) {
        hardTimer = setTimeout(() => rejectForTimeout('hard'), request.hardTimeoutMs);
      }
    });
  }
...
function formatTimeoutError(agentId: string, limits: CLIAgentRunLimits, kind: 'idle' | 'hard'): string {
  return kind === 'idle'
