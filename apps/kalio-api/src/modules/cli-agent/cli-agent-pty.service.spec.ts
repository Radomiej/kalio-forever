...
describe('CLIAgentPtyService', () => {
  ...
  it('kills the PTY process on inactivity timeout', async () => {
    vi.useFakeTimers();
    const proc = makePtyProcess();
    spawn.mockReturnValue(proc);
    ...
    await expect(run).rejects.toThrow('idle timed out after 100ms');
    expect(proc.kill).toHaveBeenCalledWith('SIGTERM');
    expect(proc.dataDisposer.dispose).toHaveBeenCalled();
    expect(proc.exitDisposer.dispose).toHaveBeenCalled();
  });

  it('uses Windows taskkill process-tree termination on inactivity timeout when running on win32', async () => {
    vi.useFakeTimers();
    const proc = makePtyProcess();
    spawn.mockReturnValue(proc);
