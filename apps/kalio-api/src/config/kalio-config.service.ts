...
export class KalioConfigService {
  ...
  async getToolTimeoutSettings(): Promise<Partial<ToolTimeoutSettings>> {
    ...
    if (
      typeof toolTimeouts.provider_max_concurrent_streams === 'number'
      && Number.isFinite(toolTimeouts.provider_max_concurrent_streams)
    ) {
      result.providerMaxConcurrentStreams = Math.round(toolTimeouts.provider_max_concurrent_streams);
    }
  ...
  private async readConfigFile(filePath: string): Promise<KalioConfig | null> {
    ...
    try {
      ...
      throw new Error(`Failed to parse Kalio config at ${filePath}: ${message}`, { cause: error });
    }
  }
